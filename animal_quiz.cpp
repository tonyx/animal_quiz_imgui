...
char* Knowledge_tree_ref::get_buffer() {
    return this->buffer;
}

void Knowledge_tree_ref::set_buffer(char* pre_buffer,char* in_buffer) {
    strcpy(buffer,pre_buffer);
    char* endofstring = strchr(buffer,'\0');
    strcpy(endofstring,in_buffer);
}
